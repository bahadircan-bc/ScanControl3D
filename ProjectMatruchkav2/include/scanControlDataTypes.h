...
#define ERROR_POSTPROCESSING_REFLECTION_NUMBER_TOO_HIGH -205
#define ERROR_POSTPROCESSING_REFLECTION_NUMBER_NOT_AVAILABLE -206
#define ERROR_POSTPROCESSING_INVALID_CONTAINER_RESOLUTION -207
#define ERROR_POSTPROCESSING_INCOMPLETE_M0_OR_M1 -208
...
// Defines for the return values of the ConvertProfile2Values,
// ConvertPartProfile2Values and ConvertContainer2Values functions
...
typedef struct TPartialProfile
{
 ...
 typedef struct TConvertContainerParameter
{
  const unsigned char* pContainer;
  DWORD  profileRearrangement;
  unsigned int numberOfProfilesToExtract;
  TScannerType scannerType;
  unsigned int reflectionNumber;
  bool convertToMM;
  unsigned short* pReflectionWidth;
  unsigned short* pMaxIntensity;
  unsigned short* pThreshold;
  unsigned int* pMoment0;
  unsigned int* pMoment1;
  double* pX;
  double* pZ;
} TConvertContainerParameter;
